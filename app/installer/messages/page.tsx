...
export default function InstallerMessagesPage() {
  ...
  useEffect(() => {
    ...
    void loadMessages(savedName);
  }, []);

  async function loadMessages(savedName: string) {
    setLoading(true);
    ...
    const installer = savedName.trim().toLowerCase();

    const filtered = ((data as MessageItem[]) || []).filter((item) => {
      return (item.installer_name || "").trim().toLowerCase() === installer;
    });
  ...
  async function handleSend() {
    if (!installerName.trim()) {
      alert("Please enter your installer name on the dashboard first.");
      return;
    }
    ...
    await loadMessages(installerName);
    setSending(false);
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        <div className="mt-4 rounded-xl border border-zinc-800 bg-black p-4">
          ...
          <p className="mt-1 font-semibold text-white">
            {installerName || "Not set from dashboard yet"}
          </p>
        </div>
      </section>
