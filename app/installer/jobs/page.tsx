...
type DerivedPayout = {
  ...
  returnPay: number;
  totalPay: number;
  payoutLines: ParsedLine[];
  parsedAddons: ParsedLine[];
  parsedJustServices: ParsedLine[];
};
...
function parseAddons(job: Booking): ParsedLine[] {
  const addOns = toArray(job.add_on_services);
  const results: ParsedLine[] = [];

  const waterfallQuantity = Math.max(1, num(job.waterfall_quantity) || 1);
  const outletQuantity = Math.max(1, num(job.outlet_plug_cutout_quantity) || 1);
  const disposal = safeText(job.disposal_responsibility).toLowerCase();

  addOns.forEach((item) => {
    const label = normalizeText(item);
    const qty = Math.max(1, getQuantityFromText(item));

    if (label.includes("waterfall")) {
      if (!results.some((r) => r.label.startsWith("Waterfall"))) {
        results.push({
          label: `Waterfall x${waterfallQuantity}`,
          amount: round2(waterfallQuantity * ADDON_RATE_WATERFALL),
        });
      }
      return;
    }
    ...
    if (
      label.includes("outletplugcutout") ||
      label.includes("plugcutout") ||
      label.includes("outlet")
    ) {
      if (!results.some((r) => r.label.startsWith("Outlet Plug Cutout"))) {
        results.push({
          label: `Outlet Plug Cutout x${outletQuantity}`,
          amount: round2(outletQuantity * ADDON_RATE_OUTLET_CUTOUT),
        });
      }
      return;
    }

    if (label.includes("difficult") || label.includes("stairs") || label.includes("basement")) {
      results.push({
        label: "Difficult / Stairs / Basement",
        amount: ADDON_RATE_DIFFICULT,
      });
      return;
    }
    ...
    if (label.includes("extrahelper")) {
      results.push({
        label: `Extra Helper x${qty}`,
        amount: round2(qty * ADDON_RATE_EXTRA_HELPER),
      });
      return;
    }

    if (label.includes("condo") || label.includes("highrise") || label.includes("high-rise")) {
      results.push({
        label: "Condo / High-Rise",
        amount: ADDON_RATE_CONDO,
      });
      return;
    }
    ...
    if (label.includes("sinkcutout")) {
      results.push({
        ...
        amount: ADDON_RATE_SINK_CUTOUT_ONSITE,
      });
      return;
    }
    ...
    if (label.includes("cooktop")) {
      results.push({
        ...
        amount: ADDON_RATE_COOKTOP_CUTOUT,
      });
      return;
    }
    ...
    if (label.includes("sealing")) {
      results.push({
        label: "Granite / Marble Sealing",
        amount: ADDON_RATE_SEALING,
      });
      return;
    }
    ...
    if (label.includes("plumbing")) {
      results.push({
        label: "Plumbing Removal / Service",
        amount: ADDON_RATE_PLUMBING_REMOVAL,
      });
      return;
    }
    ...
    if (label.includes("vanityremoval")) {
      results.push({
        label:
          disposal === "installer"
            ? "Vanity Removal Installer Disposal"
            : "Vanity Removal Customer Disposal",
        amount:
          disposal === "installer"
            ? ADDON_RATE_VANITY_REMOVAL_INSTALLER_DISPOSAL
            : ADDON_RATE_VANITY_REMOVAL_CUSTOMER_DISPOSAL,
      });
      return;
    }
    ...
    if (label.includes("onsitecutting") || label.includes("on-sitecutting")) {
      results.push({
        ...
        amount: ADDON_RATE_ONSITE_CUTTING,
      });
      return;
    }
    ...
    if (label.includes("onsitepolishing") || label.includes("on-sitepolishing")) {
      results.push({
        ...
        amount: ADDON_RATE_ONSITE_POLISHING,
      });
    }
  });
...
function parseJustServices(job: Booking): ParsedLine[] {
  const justServices = toArray(job.just_services);
  const results: ParsedLine[] = [];
  const disposal = safeText(job.disposal_responsibility).toLowerCase();

  justServices.forEach((item) => {
    const label = normalizeText(item);

    if (label.includes("remeasurebacksplashfh")) {
      results.push({ label: "Remeasure Backsplash FH", amount: ADDON_RATE_REMEASURE_FH });
      return;
    }

    if (label.includes("remeasurebacksplashlh")) {
      results.push({ label: "Remeasure Backsplash LH", amount: ADDON_RATE_REMEASURE_LH });
      return;
    }

    if (label.includes("reattachsink") || label.includes("sinkreattach")) {
      results.push({ label: "Reattach Sink", amount: ADDON_RATE_REATTACH_SINK });
      return;
    }

    if (label.includes("sinkcutout")) {
      results.push({ label: "Sink Cutout Onsite", amount: ADDON_RATE_SINK_CUTOUT_ONSITE });
      return;
    }

    if (label.includes("cooktop")) {
      results.push({ label: "Cooktop Cutout", amount: ADDON_RATE_COOKTOP_CUTOUT });
      return;
    }

    if (label.includes("plumbing")) {
      results.push({ label: "Plumbing Removal / Service", amount: ADDON_RATE_PLUMBING_REMOVAL });
      return;
    }

    if (label.includes("sealing")) {
      results.push({ label: "Granite / Marble Sealing", amount: ADDON_RATE_SEALING });
      return;
    }

    if (label.includes("onsitecutting") || label.includes("on-sitecutting")) {
      results.push({ label: "Onsite Cutting", amount: ADDON_RATE_ONSITE_CUTTING });
      return;
    }

    if (label.includes("onsitepolishing") || label.includes("on-sitepolishing") || label.includes("polishing")) {
      results.push({ label: "Onsite Polishing", amount: ADDON_RATE_ONSITE_POLISHING });
      return;
    }
...
function buildDerivedInstallerPayout(job: Booking): DerivedPayout {
  ...
  let basePay =
    num(job.installer_base_pay) > 0 ? num(job.installer_base_pay) : 0;

  if (basePay <= 0) {
    if (serviceType === "installation_3cm") {
      basePay = round2(sqft * INSTALLER_RATE_3CM);
    } else if (
      serviceType === "installation_2cm_standard" ||
      serviceType === "installation_2cm"
    ) {
      basePay = round2(sqft * INSTALLER_RATE_2CM);
    } else if (serviceType === "full_height_backsplash") {
      basePay = round2(sqft * INSTALLER_RATE_FULL_HEIGHT_BS);
    }
  }

  const parsedAddons = parseAddons(job);
  const parsedJustServices = parseJustServices(job);

  const addonPay =
    num(job.installer_addon_pay) > 0
      ? num(job.installer_addon_pay)
      : round2(parsedAddons.reduce((sum, item) => sum + item.amount, 0));

  const mileagePay =
    num(job.installer_mileage_pay) > 0
      ? num(job.installer_mileage_pay)
      : round2(num(job.chargeable_km || job.round_trip_km) * INSTALLER_MILEAGE_RATE);

  let cutPolishPay = num(job.installer_cut_polish_pay);
  let sinkPay = num(job.installer_sink_pay);
  let otherPay = num(job.installer_other_pay);

  if (cutPolishPay <= 0) {
    cutPolishPay = round2(
      parsedAddons
        .filter((item) => item.label.includes("Onsite Cutting") || item.label.includes("Onsite Polishing"))
  ...
  if (sinkPay <= 0) {
    sinkPay = round2(
      parsedAddons
        .filter((item) => item.label.includes("Reattach Sink"))
        .reduce((sum, item) => sum + item.amount, 0) +
        parsedJustServices
          .filter((item) => item.label.includes("Reattach Sink"))
          .reduce((sum, item) => sum + item.amount, 0)
  ...
  if (otherPay <= 0) {
    otherPay = round2(
      parsedJustServices
        .filter(
          (item) =>
            !item.label.includes("Onsite Cutting") &&
            !item.label.includes("Onsite Polishing") &&
            !item.label.includes("Reattach Sink")
        )
        .reduce((sum, item) => sum + item.amount, 0)
    );
  }
  ...
  const subtotalPay =
    num(job.installer_subtotal_pay) > 0
      ...
      : round2(basePay + addonPay + cutPolishPay + sinkPay + otherPay + mileagePay);
  ...
  const calculatedTotal = round2(subtotalPay + hstPay + returnPay);

  const totalPay =
    num(job.installer_pay) > 0 && Math.abs(num(job.installer_pay) - calculatedTotal) < 0.01
...
function getDerivedInstallerPay(job: Booking) {
  return buildDerivedInstallerPayout(job).totalPay;
}
...
function getPayoutLines(job: Booking) {
  ...
  return buildDerivedInstallerPayout(job).payoutLines;
}
...
function getAiInsight(job: Booking, allJobs: Booking[]): AiJobInsight {
  ...
  const distanceTier =
    ...
    (oneWayKm > 120 ? "Long Distance" : oneWayKm > 70 ? "Extended Range" : "Standard Zone");

  let recommendedInstallerType = job.ai_recommended_installer_type || "Standard Installer";
  ...
  const nearbyCount = allJobs.filter((item) => {
    ...
    if (
      normalizeBookingStatus(item) === "completed" ||
      normalizeBookingStatus(item) === "incomplete"
    ) {
      return false;
    }
...
async function sendAdminAcceptedEmail(job: Booking, installerName: string) {
  const payout = buildDerivedInstallerPayout(job);
  ...
  <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111;">
    ...
    <p><strong>Installer Pay:</strong> ${money(payout.totalPay)}</p>
  </div>
  `;
...
function JobDetailBlock({
  ...
  const pickupDisplay = showFullAddress ? job.pickup_address || "-" : getCityOnly(job.pickup_address);
  const dropoffDisplay = showFullAddress ? job.dropoff_address || "-" : getCityOnly(job.dropoff_address);
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex-1">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            />
            <CompactStat label="Distance" value={`${num(job.one_way_km).toFixed(2)} km`} />
            <CompactStat label="Distance Tier" value={ai.distanceTier} />
            <CompactStat label="Total Pay (incl. HST)" value={money(payout.totalPay)} />
          </div>
      ...
      {expanded ? (
        <div className="mt-5 border-t border-zinc-800 pt-5">
          ...
          <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950 p-3">
            ...
            <div className="space-y-1 text-sm text-gray-300">
              <p>Base Pay: {money(payout.basePay)}</p>
              <p>Add-On Pay: {money(payout.addonPay)}</p>
              <p>Other Service Pay: {money(payout.otherPay)}</p>
              <p>Cut / Polish Pay: {money(payout.cutPolishPay)}</p>
              <p>Sink / Reattach Pay: {money(payout.sinkPay)}</p>
              <p>Mileage Pay: {money(payout.mileagePay)}</p>
              <p>Subtotal Pay: {money(payout.subtotalPay)}</p>
              <p>HST Pay: {money(payout.hstPay)}</p>
              {payout.returnPay > 0 ? <p>Return Pay: {money(payout.returnPay)}</p> : null}
              ...
              <p className="pt-2 font-semibold text-yellow-400">
                Total Pay (incl. HST): {money(payout.totalPay)}
              </p>
            </div>
          </div>
        </div>
...
export default function InstallerJobsPage() {
  ...
  async function loadJobs() {
    ...
    try {
      ...
      const safeJobs = ((data as Booking[]) || [])
        .filter((job) => job.is_archived !== true)
        .map((job) => {
          const payout = buildDerivedInstallerPayout(job);
          return {
            ...job,
            installer_pay: payout.totalPay,
            one_way_km: num(job.one_way_km),
            sqft: num(job.sqft),
            job_size: num(job.job_size),
            installer_subtotal_pay: payout.subtotalPay,
            installer_hst_pay: payout.hstPay,
            installer_base_pay: payout.basePay,
            installer_mileage_pay: payout.mileagePay,
            installer_addon_pay: payout.addonPay,
            installer_other_pay: payout.otherPay,
            installer_cut_polish_pay: payout.cutPolishPay,
            installer_sink_pay: payout.sinkPay,
          };
        });
  ...
  async function acceptSingleJob(job: Booking) {
    ...
    try {
      ...
      try {
        await sendAdminAcceptedEmail(
          { ...freshJob, installer_name: installerName.trim(), accepted_at: acceptedAt },
          installerName.trim()
        );
      } catch (emailError) {
  ...
  const groupedJobs = useMemo(() => {
    ...
    const result: GroupedJobs[] = Array.from(groups.entries()).map(
      ([groupKey, groupItems]) => ({
        groupKey,
        jobs: [...groupItems].sort((a, b) => num(a.job_number) - num(b.job_number)),
      })
    );
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        {installerName ? (
          <p className="mt-3 text-sm text-zinc-300">
            Logged in as: <span className="font-semibold text-yellow-400">{installerName}</span>
          </p>
        ) : null}
      </div>
          ...
          {nextDayGroups.length > 0 ? (
            <section className="space-y-4">
              <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/10 p-5">
                <h2 className="text-2xl font-bold text-yellow-400">Next-Day Priority Jobs</h2>
          ...
          {regularGroups.length > 0 ? (
            <section className="space-y-4">
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
                <h2 className="text-2xl font-bold text-yellow-400">All Other Jobs</h2>
                <p className="mt-1 text-sm text-zinc-300">
                  Available and assigned jobs with full AI route and payout guidance.
                </p>
              </div>
